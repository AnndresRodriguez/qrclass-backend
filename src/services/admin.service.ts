...
class AdminService {
    ...
    async desactivateAdmin(idAdmin: number) {
      ...
      if (_.isNumber(+idAdmin)) {
        ...
        if (adminToDisable !== undefined){
          ...
          adminToDisable.estado = adminToDisable.estado == 0 ? adminToDisable.estado = 1 : adminToDisable.estado = 0;
    ...
    async validateDocumentAdmin(documento: string): Promise<boolean> {
      ...
      const docenteFinded = await docenteRepository.find({ where: { documento: documento } });
